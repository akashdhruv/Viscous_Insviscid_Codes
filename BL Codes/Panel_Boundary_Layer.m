...
for j=1:Na/2
    ...
    bxu(j) = (deltaUpper(j)*cos(beta)*10+xj_d(i));
    byu(j) = (deltaUpper(j)*sin(beta)*10+zj_d(i));
    ...
    bxl(j) = (deltaLower(j)*cos(beta)*10+xco(j));
    byl(j) = (deltaLower(j)*sin(beta)*10+zco(j));
end
%%
figure
hold on
plot(xj_d(1:Na+1),zj_d(1:Na+1),'k')
plot(bxu(1:end-4),byu(1:end-4),'b')
plot(bxl(1:end-4),byl(1:end-4),'b')
axis equal
